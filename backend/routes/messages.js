...
router.get("/stream", authenticateToken, (req, res) => {
  ...
  req.on("close", () => {
    ...
    const activeConnections = userConnections.filter(client => client !== res);
...
router.post("/broadcast", authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    if (targetUserIds && targetUserIds.length > 0) {
      // Specific users
      usersToReceive = await prisma.user.findMany({ where: { id: { in: targetUserIds } } });
    } else {
    ...
    res.json({ success: true, message: `Message dispatched to ${usersToReceive.length} inboxes.` });
  } catch (error) {
